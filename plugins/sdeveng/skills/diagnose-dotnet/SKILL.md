...
Start with the reported symptom and any caller-provided artifact. Run `dotnet diagnostics-plan` with the matching `--signal`, PID when known, and the shortest useful duration. It determines OS, runtime, SDK, installed tools, artifact path, and a non-executing collection command. Inspect an existing trace, dump, or GC dump before collecting again.
