...
Require a reproducible workload and baseline. Use `sdeveng dotnet inspect --json` and `sdeveng dotnet build-plan --binlog --json`; compare compatible clean or incremental runs without changing multiple variables. Query binlogs structurally for evaluation, target/task duration, critical path, skipped targets, and project-reference scheduling before using raw logs.
