...
Record the workload, symptom, baseline, environment, and measurement window. Reuse comparable counters, traces, profiles, or benchmark results first. Run `sdeveng dotnet diagnostics-plan --json` only for the narrow missing signal; it owns collection mechanics and keeps binary artifacts under `.agent-tool/traces`.
