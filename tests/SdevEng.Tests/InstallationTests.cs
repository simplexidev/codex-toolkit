...
public class InstallationTests
{
    ...
    [Fact] public void InstallUpdateIdempotentAndUninstallPreservesReplacement() { using var repo = new TemporaryGitRepository(); var home = Path.Combine(repo.Root, "home"); var bin = !OperatingSystem.IsWindows(); Assert.Equal(0, Installer.Run(Toolkit, home, null, "install", false, bin).ExitCode); Assert.Equal(0, Installer.Run(Toolkit, home, null, "update", false, bin).ExitCode); var file = Path.Combine(home, ".codex/AGENTS.md"); File.Delete(file); File.WriteAllText(file, "my replacement"); Assert.Equal(0, Installer.Run(Toolkit, home, null, "uninstall", false, false).ExitCode); Assert.Equal("my replacement", File.ReadAllText(file)); if (bin) Assert.False(File.Exists(Path.Combine(home, ".local/bin/codex-agent-tool"))); }
